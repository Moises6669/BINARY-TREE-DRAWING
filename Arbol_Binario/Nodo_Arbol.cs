...
namespace Arbol_Binario_Moises_Masis
{
    class Nodo_Arbol
    {
        public double info; // Dato a almacenar en el nodo
        ...
        public Arbol_Binario Arbol1 { get => arbol; set => arbol = value; }

        public Nodo_Arbol(double nueva_info, Nodo_Arbol izquierdo, Nodo_Arbol derecho,
        Nodo_Arbol padre)
        {
            info = nueva_info;
            Izquierdo = izquierdo;
        ...
        public Nodo_Arbol Insertar(double dato, Nodo_Arbol t, int Level, Nodo_Arbol raiz = null)
        {
            if (t == null)
            {
                t = new Nodo_Arbol(dato, null, null, null);
                t.nivel = Level;
            }
            else if (dato < t.info)
            {
                Level++;
                t.Izquierdo = Insertar(dato, t.Izquierdo, Level);
            }
            else if (dato > t.info)
            {
                Level++;
                t.Derecho = Insertar(dato, t.Derecho, Level);
            }
            else
            {
        ...
        public void Eliminar(double x, ref Nodo_Arbol t)
        {
            ...
            {
                MessageBox.Show(" Nodo no encontrado Intente con coma en vez de punto", "Error de Eliminacion",MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        } // Final de la función Eliminar
          // Función para recorrer el Árbol Binario (búsqueda de un
